...
def game_scene():
    ...
    class Ball(stage.Sprite):
        ...
        def update(self):
            ...
            for sprite in sprites:
                ...
                for berry_number in range(len(berries)):
                    if berries[berry_number].x > 0:
                        if stage.collide(ball1.x, ball1.y,
                                         ball1.x + 15, ball1.y + 15,
                                         berries[berry_number].x, berries[berry_number].y,
                                         berries[berry_number].x + 16, berries[berry_number].y + 16):
                            
                            # ball hit a strawberry
                            berries[berry_number].move(constants.OFF_SCREEN_X, constants.OFF_SCREEN_Y)
                            berries[berry_number].move(constants.OFF_SCREEN_X, constants.OFF_SCREEN_Y)
    ...
    game.layers = [score_text] + berries + sprites + [paddle] + [background]
    # render all sprites
    game.render_block()
